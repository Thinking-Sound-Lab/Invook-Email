...
4. One structured per-message classifier independently evaluates the built-in Newsletter definition and active custom Invook labels. Creating or editing a definition does not scan historical mail. Important comes only from Gmail, Others is derived after successful analysis, and opaque Gmail user-label IDs are ignored. Applied relationships and explicit user overrides are persisted per message; thread display is calculated from visible messages.
