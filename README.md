...
Temporal execution is isolated by Invook user. Every user receives deterministic `control`, `live`, and `bulk` task queues; all Gmail accounts owned by that user share those queues. Gmail history catch-up and Workflow Tasks use `control`, incremental AI work uses `live`, and snapshots and historical derivations use `bulk`. This prevents one user's historical mailbox synchronization from consuming another user's Activity capacity and lets new Gmail history bypass the same user's bulk backlog.
