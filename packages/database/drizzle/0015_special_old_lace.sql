DO $$
BEGIN
	LOCK TABLE "mail_sync_runs", "workflow_steps", "gmail_sync_items", "queue_outbox" IN ACCESS EXCLUSIVE MODE;
	...
	ALTER TABLE "mail_sync_runs" DROP CONSTRAINT "mail_sync_runs_status_check";
	...
	CREATE UNIQUE INDEX "mail_sync_runs_single_active_account_idx"
		ON "mail_sync_runs" USING btree ("account_id")
		WHERE "mail_sync_runs"."status" in ('queued', 'running');

	ALTER TABLE "mail_sync_runs"
		ADD CONSTRAINT "mail_sync_runs_status_check"
		CHECK ("mail_sync_runs"."status" in ('queued', 'running', 'complete', 'failed', 'superseded'));
END $$;
