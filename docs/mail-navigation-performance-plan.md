...
- Durable payloads already carry `changedThreadIds` for history and label-analysis events (`packages/database/src/replica.ts:839-843`, `packages/database/src/message-label-analysis.ts:226-229`), so targeted invalidation for those event types needs no new durable state and no new worker checkpoint. It is not free of work: Phase 4 still routes every producer through a shared typed insertion boundary and adds `refreshedThreadIds` to the message-refresh producer. That does not extend to draft events, which carry no thread or draft identifier at all.
...
- `labels_changed` has two producers, both carrying `messageId` and `changedThreadIds`, one adding `labelAnalysisState` (`message-label-analysis.ts:226-229`) and one adding `labelId` (`message-label-analysis.ts:944-947`).
...
- Give every durable producer a typed contract first. `insertMailboxChange` accepts `payload?: Record<string, unknown>` today (`packages/database/src/replica.ts:242`), while label-analysis producers insert into `mailboxChangeEvents` directly (`packages/database/src/message-label-analysis.ts:221-230`, `939-950`), so changing the replica-local helper alone would leave untyped writers. Move the event input union and insertion operation to one feature-owned database module, route every product producer through it, and leave raw table inserts only in migrations or tests. Model the real semantic variants using `changeType` plus a required secondary discriminator such as `reason` or `kind`. Do not force unrelated producers into optional fields merely to make `changeType` the only discriminator. At minimum, distinguish history catch-up from message refresh, label-analysis resolution from label-decision change, and draft snapshot from draft upsert or deletion.
...
- **View relevance is not derivable from thread identifiers alone.** A thread that newly gains `IMPORTANT` is absent from the client's current list, so the client cannot judge relevance. Either the event carries the affected view or label categories, or the client asks the server whether a change intersects the visible view. Do not silently fall back to refreshing the list on every event; that reproduces today's behavior with more machinery.
...
- **A thread with new mail is absent from `changedThreadIds`.** A message upsert adds its thread only when analysis was not queued (`packages/database/src/replica.ts:780`), so arrival and the reordering it causes are announced by the later `labels_changed` event (`packages/database/src/message-label-analysis.ts:226-229`). Map that event to the thread list explicitly. Today's whole-route refresh hides the dependency; targeted invalidation drops new mail from the list if the mapping is missing.
