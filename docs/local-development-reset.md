...
After clearing the stores, the command verifies the zero-data state while API, web, and worker services are still stopped, so an authenticated inbound webhook cannot race the destructive operation's result. It fails unless all product tables and mailbox objects are empty and the Drizzle migration row count is unchanged. It then rebuilds and restarts the normal stack. Its final output includes zero counts for profiles, connected accounts, messages, embeddings, drafts, memories, operation checkpoints, and Temporal commands.
